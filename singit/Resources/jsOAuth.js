...
exports.OAuth = function(global) {
    ...
    function QueryString(obj) {
        ...
        if (this instanceof args_callee) {
            if (obj != undefined) for (i in obj) obj.hasOwnProperty(i) && (querystring[i] = obj[i]);
            return querystring;
        }
        return new args_callee(obj);
    }
    function OAuth(options) {
        return this instanceof OAuth ? this.init(options) : new OAuth(options);
    }
    function toHeaderString(params) {
        var arr = [], i, realm;
        for (i in params) params[i] && params[i] !== undefined && params[i] !== "" && (i === "realm" ? realm = i + "=\"" + params[i] + "\"" : arr.push(i + "=\"" + OAuth.urlEncode(params[i] + "") + "\""));
        arr.sort();
    ...
    function toSignatureBaseString(method, url, header_params, query_params) {
        ...
        for (i in header_params) header_params[i] !== undefined && header_params[i] !== "" && arr.push([ OAuth.urlEncode(i), OAuth.urlEncode(header_params[i] + "") ]);
        for (i in query_params) query_params[i] !== undefined && query_params[i] !== "" && (header_params[i] || arr.push([ encode(i), encode(query_params[i] + "") ]));
    ...
    function getNonce(key_length) {
        ...
        for (i = 0; i < key_iter; i++) value += chars[rand()] + chars[rand()] + chars[rand()] + chars[rand()];
        for (i = 0; i < key_remainder; i++) value += chars[rand()];
        return value;
    }
    function Request() {
        var XHR;
        typeof global.Titanium != "undefined" && typeof global.Titanium.Network.createHTTPClient != "undefined" ? XHR = global.Titanium.Network.createHTTPClient() : typeof require != "undefined" ? XHR = (new require("xhr")).XMLHttpRequest() : XHR = new global.XMLHttpRequest;
        return XHR;
    }
    ...
    function stringToByteArray(str) {
        ...
        for (i = 0; i < str.length; i++) {
            ...
            code < 128 ? bytes.push(code) : code < 2048 ? bytes.push(192 + (code >> 6), 128 + (code & 63)) : code < 65536 ? bytes.push(224 + (code >> 12), 128 + (code >> 6 & 63), 128 + (code & 63)) : code < 2097152 && bytes.push(240 + (code >> 18), 128 + (code >> 12 & 63), 128 + (code >> 6 & 63), 128 + (code & 63));
        }
        return bytes;
    }
    function wordsToByteArray(words) {
        var bytes = [], i;
        for (i = 0; i < words.length * 32; i += 8) bytes.push(words[i >>> 5] >>> 24 - i % 32 & 255);
        return bytes;
    }
    function byteArrayToHex(byteArray) {
        ...
        for (i = 0; i < l; i++) {
            ...
            hex.push((byteArray[i] & 15).toString(16));
        }
        return hex.join("");
    }
    function byteArrayToString(byteArray) {
        var string = "", l = byteArray.length, i;
        for (i = 0; i < l; i++) string += String.fromCharCode(byteArray[i]);
        return string;
    }
    ...
    function SHA1(message) {
        if (message !== undefined) {
            ...
            this instanceof SHA1 ? crypto = this : crypto = new SHA1(message);
            digest = crypto.hash(m);
            return byteArrayToHex(digest);
        }
        return this instanceof SHA1 ? this : new SHA1;
    }
    function HMAC(fn, key, message, toHex) {
        ...
        return toHex ? byteArrayToHex(byteArray) : byteArrayToString(byteArray);
    }
    Hash.prototype = {
        ...
        ksort: function(func) {
            ...
            func == undefined ? keys.sort() : keys.sort(func);
    ...
    QueryString.prototype.toString = function() {
        ...
        self.ksort();
        for (i in self) if (self.hasOwnProperty(i) && i != undefined && self[i] != undefined) {
    ...
    QueryString.prototype.setQueryParams = function(query) {
        ...
        if (args_length == 1) {
            if (typeof query == "object") for (i in query) query.hasOwnProperty(i) && (querystring[i] = query[i]); else if (typeof query == "string") {
        ...
        } else for (i = 0; i < arg_length; i += 2) querystring[args[i]] = args[i + 1];
    };
    ...
    OAuth.prototype = {
        ...
        init: function(options) {
            var empty = "", oauth = {
                enablePrivilege: options.enablePrivilege || !1,
            ...
            this.request = function(options) {
                ...
                withFile = function() {
                    var hasFile = !1;
                    for (var name in data) typeof data[name].fileName != "undefined" && (hasFile = !0);
                    return hasFile;
                }();
                appendQueryString = options.appendQueryString ? options.appendQueryString : !1;
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if (!xhr.getAllResponseHeaders) {
                            if (!!xhr.getResponseHeaders) {
                                ...
                                for (var i = 0, len = responseHeadersString.length; i < len; ++i) responseHeaders[responseHeadersString[i][0]] = responseHeadersString[i][1];
                            }
                        } else {
                        ...
                        var includeXML = !1;
                        "Content-Type" in responseHeaders && responseHeaders["Content-Type"] == "text/xml" && (includeXML = !0);
                        var responseObject = {
                        ...
                        xhr.status >= 200 && xhr.status <= 226 || xhr.status == 304 || xhr.status === 0 ? success(responseObject) : xhr.status >= 400 && xhr.status !== 0 && failure(responseObject);
                ...
                } else if (typeof global.Titanium != "undefined") query = data; else {
                ...
                "Content-Type" in headers && typeof global.Titanium != "undefined" && global.Titanium.Platform.osname === "android" && delete headers["Content-Type"];
        ...
        parseTokenRequest: function(tokenRequest, content_type) {
            switch (content_type) {
              case "text/xml":
                var token = tokenRequest.xml.getElementsByTagName("token"), secret = tokenRequest.xml.getElementsByTagName("secret");
    ...
    OAuth.urlEncode = function(string) {
        ...
        for (i = 0; i < str_len; i++) if (c = string_arr[i].match(reserved_chars)) {
            c = c[0].charCodeAt(0);
            c < 128 ? string_arr[i] = hex(c) : c < 2048 ? string_arr[i] = hex(192 + (c >> 6)) + hex(128 + (c & 63)) : c < 65536 ? string_arr[i] = hex(224 + (c >> 12)) + hex(128 + (c >> 6 & 63)) + hex(128 + (c & 63)) : c < 2097152 && (string_arr[i] = hex(240 + (c >> 18)) + hex(128 + (c >> 12 & 63)) + hex(128 + (c >> 6 & 63)) + hex(128 + (c & 63)));
        }
    ...
    SHA1.prototype.hash = function(m) {
        ...
        var H = [ 1732584193, 4023233417, 2562383102, 271733878, 3285377520 ], K = [ 1518500249, 1859775393, 2400959708, 3395469782 ], lb, hb, l, pad, ml, blocks, b, block, bl, w, i, A, B, C, D, E, t, n, TEMP;
        ...
        for (b = 0; b < blocks; b++) {
            ...
            bl = block.length;
            w = [];
            for (i = 0; i < bl; i++) w[i >>> 2] |= block[i] << 24 - (i - (i >> 2) * 4) * 8;
            A = H[0];
            B = H[1];
            C = H[2];
            D = H[3];
