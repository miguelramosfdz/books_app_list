...
function apiCall(_options, _callback) {
    ...
    if (_options.type == "GET") xhr.send(); else {
        var params = _options.data ? JSON.parse(_options.data) : null;
        xhr.send(params);
    }
}
...
function Sync(model, method, opts) {
    ...
    switch (method) {
      ...
      case "read":
        ...
        if (params.data) {
            var querys = [];
            _.map(_.pairs(params.data), function(arr) {
                querys.push(arr[0] + "=" + arr[1]);
            });
            params.url += "?" + querys.join("&");
        }
        apiCall(params, function(_response) {
            if (_response.success) {
...
var TAP = Ti.App.Properties, _ = require("alloy/underscore")._;
